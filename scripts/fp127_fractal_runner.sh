#!/usr/bin/env bash
# Build + run the Fractal-over-Fp2_M127 runner for the BDEC-R1CS wire format.
#
...
#
# Usage:
#   scripts/fp127_fractal_runner.sh <wire-file>                  # build + run
#   SKIP_BUILD=1 scripts/fp127_fractal_runner.sh <wire-file>     # reuse prior build
...
OUT_BIN="target/fp127_fractal_runner"
mkdir -p target
...
if [[ -z "${SKIP_BUILD:-}" ]]; then
  ...
  "$CXX" -std=gnu++14 -O2 -g \
    "${DEFS[@]}" "${INCS[@]}" \
    scripts/fp127_fractal_runner.cpp \
    "$LIBIOP_A" "$LIBFF_A" \
