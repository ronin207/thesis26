/** @file
 *  Fractal / libiop runner for the BDEC-R1CS wire format v1 ("BR1S").
 *
 *  Reads a .bdec-r1cs file produced by `src/bin/emit_aurora_r1cs.rs`
 *  (same wire format as the Aurora runner), constructs a
 *  libiop::r1cs_constraint_system<fp127_Fq2>, runs the Fractal holographic
 *  SNARK sequence (indexer + prover + verifier) and prints a one-line JSON
 *  report suitable for automation.
 *
 *  Fractal is a PREPROCESSING (holographic) SNARK: it has a per-circuit
 *  indexer whose wall-clock IS R_static (the recompile cost). The prover is
 *  per-proof. We therefore time `fractal_snark_indexer` separately and emit
 *  it as the field "t_index_ms".  Contrast with Aurora, which is
 *  non-preprocessing (R_static ~= microseconds of param-setup).
 *
 *  Build / run: scripts/fp127_fractal_runner.sh
 */
...
int main(int argc, char **argv)
{
    ...
    // corrupt the witness, skip the precheck, and confirm Fractal REJECTs.
    ...
    if (!is_sat) {
        ...
        std::cerr << "[runner] WARNING: R1CS UNSATISFIED (SKIP_SATISFIED_CHECK=1) "
                     "-- proceeding; Fractal verifier should REJECT.\n";
    }

    // Fractal parameters: mirror the Aurora runner's safe defaults plus the
    // upstream Fractal test/profiling call pattern (test_fractal_snark.cpp,
    // instrument_fractal_snark.cpp). Caller can flip ZK via env.
    const size_t security_level      = 128;
    const bool   make_zk             = std::getenv("ZK") != nullptr;
    // Match the Aurora runner: RS_extra_dimensions=5, fri_localization=2.
    const size_t RS_extra_dimensions = 5;
    const size_t fri_localization    = 2;   // size_t -> single-value overload
    std::cerr << "[runner] make_zk=" << (make_zk ? "true" : "false")
              << " RS_extra_dimensions=" << RS_extra_dimensions << "\n";

    // Fractal's parameter constructor takes a shared_ptr to the constraint
    // system (it is holographic, so the circuit is bound at param-setup time).
    auto cs_ptr =
        std::make_shared<libiop::r1cs_constraint_system<FieldT>>(loaded.cs);

    auto ts0 = std::chrono::steady_clock::now();
    libiop::fractal_snark_parameters<FieldT, HashT> params(
        security_level,
    ...
    // --- R_static = the holographic indexer wall-clock. -----------------
    // Fractal is a PREPROCESSING SNARK: the indexer encodes the constraint
    // matrices into low-degree-extension oracles and Merkle-commits them,
    // ONCE per circuit, reused across all proofs. This is the per-circuit
    // recompile cost we are measuring -- contrast Aurora (non-preprocessing,
    // R_static ~= microseconds of param-setup).
    std::cerr << "[runner] Fractal indexer (R_static) ...\n";
    auto t_idx0 = std::chrono::steady_clock::now();
    std::pair<libiop::bcs_prover_index<FieldT, HashT>,
              libiop::bcs_verifier_index<FieldT, HashT>> index =
        libiop::fractal_snark_indexer<FieldT, HashT>(params);
    auto t_idx1 = std::chrono::steady_clock::now();
    const double t_index_ms = std::chrono::duration<double, std::milli>(t_idx1 - t_idx0).count();

    std::cerr << "RECOMPILE-COST(fractal) t_index_ms=" << t_index_ms
              << " (R_static = holographic indexer; prove is per-proof)\n";
    std::cerr << "[runner]   (param-setup t_setup_ms=" << t_setup_ms
              << ", wire-load t_load_ms=" << t_load_ms << ")\n";

    // --- prove (per-proof). NOTE: fractal_snark_prover MUTATES index.first.
    std::cerr << "[runner] Fractal prover ...\n";
    auto t0 = std::chrono::steady_clock::now();
    libiop::fractal_snark_argument<FieldT, HashT> proof =
        libiop::fractal_snark_prover<FieldT, HashT>(
            index.first, loaded.primary, loaded.auxiliary, params);
    auto t1 = std::chrono::steady_clock::now();
    const double t_P_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();

    const size_t proof_bytes = proof.size_in_bytes();

    std::cerr << "[runner] Fractal verifier ...\n";
    auto t2 = std::chrono::steady_clock::now();
    const bool ok = libiop::fractal_snark_verifier<FieldT, HashT>(
        index.second, loaded.primary, proof, params);
    auto t3 = std::chrono::steady_clock::now();
    const double t_V_ms = std::chrono::duration<double, std::milli>(t3 - t2).count();

    emit_json(path, ok, t_index_ms, t_P_ms, t_V_ms, proof_bytes, loaded,
