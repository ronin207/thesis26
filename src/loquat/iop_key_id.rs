...
use super::field_utils::{F, legendre_prf_secure, u128_to_field};
...
pub fn create_iop_instance(
    ...
    // Use Griffin hash for message hashing (SNARK-friendly)
    let message_hash = GriffinHasher::hash(message);
...
fn field_slice_to_bytes(slice: &[F]) -> Vec<u8> {
    ...
    for field_element in slice {
        bytes.extend_from_slice(&field_element.0.to_le_bytes());
    }
    bytes
}
...
pub fn verify_iop_proof(
    ...
    for i in 0..params.b {
        ...
        if legendre_prf_secure(o_i) != pk_i + t_i - u128_to_field(2) * pk_i * t_i {
            return Ok(false);
        }
    }
