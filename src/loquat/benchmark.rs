...
use super::setup::{SUPPORTED_SECURITY_LEVELS, loquat_setup, security_profile};
...
impl Default for BenchmarkConfig {
    fn default() -> Self {
        Self {
            security_level: SUPPORTED_SECURITY_LEVELS[2] as u32,
            num_iterations: 10,
...
impl LoquatBenchmark {
    ...
    pub fn new() -> Self {
        Self {
            configs: SUPPORTED_SECURITY_LEVELS
                .iter()
                .map(|&level| BenchmarkConfig {
                    security_level: level as u32,
                    num_iterations: 10,
                    message_size: 32,
                    hash_type: HashType::ShaSHAKE,
                })
    ...
    pub fn paper_config() -> Self {
        const PAPER_LEVELS: [u32; 3] = [80, 100, 128];
        let mut configs = Vec::with_capacity(PAPER_LEVELS.len() * 2);
        for &level in PAPER_LEVELS.iter() {
    ...
    pub fn sha_config() -> Self {
        const SHA_LEVELS: [u32; 3] = [80, 100, 128];
        Self {
            configs: SHA_LEVELS
                .iter()
                .map(|&level| BenchmarkConfig {
    ...
    pub fn griffin_config() -> Self {
        const GRIFFIN_LEVELS: [u32; 3] = [80, 100, 128];
        Self {
            configs: GRIFFIN_LEVELS
                .iter()
                .map(|&level| BenchmarkConfig {
                    security_level: level,
                    num_iterations: 20,
    ...
    fn estimate_query_complexity_by_security_level(&self, security_level: u32) -> usize {
        security_profile(security_level as usize)
            .map(|profile| profile.kappa)
            .unwrap_or(30)
    }
    ...
    pub fn export_csv(
        ...
        writeln!(
            file,
            "Security Level,Query Complexity,Signature Size (KB),Signing Time (ms),Verification Time (ms),Setup Time (ms),KeyGen Time (ms),Hash Type"
        )?;
