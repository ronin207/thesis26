...
# CRITICAL: default SHARD_SIZE -> ensure NOT set (falls back to 1<<24)
env.pop("SHARD_SIZE", None)
env["SP1_WORKER_NUM_RECURSION_PROVER_WORKERS"]=rw
env["SP1_WORKER_NUM_CORE_WORKERS"]=cw
# RUST_LOG: surface prover phase transitions across core->recursion->shrink->wrap->gnark
env["RUST_LOG"]="info,sp1_core_machine=info,sp1_prover=info,sp1_recursion_machine=info,p3_=warn"
env["SP1_PROVER"]="cpu"

def now(): return datetime.now(timezone.utc).strftime("%H:%M:%S")

meta=open(OUT+".meta","w")
meta.write(f"TAG={tag} RW={rw} CW={cw} SHARD_SIZE=DEFAULT(1<<24) wrap=plonk arm=syscall lambda=80\n")
...
def sampler():
    ...
    tsv.write("epoch\tutc\trss_kb\trss_gib\n")
    while not stop:
        try:
...
meta.write(f"getrusage_maxrss_bytes={maxrss_bytes} GiB={maxrss_bytes/1024**3:.3f}\n")
meta.write(f"sampler_peak_kb={peak_kb[0]} GiB={peak_kb[0]/1024/1024:.3f} sampler_peak_at_utc={peak_ts[0]}\n")
meta.flush(); meta.close()
print(f"rc={rc} wall_min={dt/60:.2f} getrusage_maxrss_GiB={maxrss_bytes/1024**3:.3f} sampler_peak_GiB={peak_kb[0]/1024/1024:.3f} peak_at={peak_ts[0]}")
