...
build drift (\S\ref{ssec:headline}). $^\ddag$RISC~Zero receipt-verify wall-clock not separately logged; the
receipt verified.}
...
BDEC ShowCre $k{=}2$, SP1 & prove (succinct) & $58.14$~min & receipt verified; not zero-knowledge \\
\bottomrule
\end{tabular}
...
process accounting at the kill; a periodic RSS sampler observed
${\approx}17.8$~GB before the final spike); this is the cost
...
bound, which is the dual obstruction. The zero-knowledge mode that is produced
...
consumer-hardware frontier of \S\ref{ssec:resource-frontier}. A zero-knowledge
proof of ShowCre at any $k\ge 1$ is therefore bounded by that wrap frontier
rather than by the CreGen prove anomaly (\S\ref{ssec:cregen-anomaly}). More
memory would clear the resource bound, but a post-quantum-anonymous proof
...
\begin{itemize}
  ...
  wrap memory exhaustion, and the Loquat substrate gap survive unchanged or
