...
$\mathsf{Adv}^{\bullet}_{\mathsf{zkVM\text{-}Cred}}$ are those of
Definition~\ref{def:zkvm-cred-adv}.
...
wrapping) are not produced on the target hardware as the toolchain ships, blocked
by the recursion-shape provisioning wall;
see Section~\ref{ssec:zk-gap}.)
\end{definition}
...
Let $\mathcal A$ win the zkVM credential unforgeability experiment. We build a PLUM
...
BDEC's unlinkability experiment (\cite[Appendix~A.3]{10.1007/978-981-96-0957-4_3},
Theorem~3) forwards a single challenge credential $c^{(b)}$ issued under one of two
pseudonym keys $\mathsf{ppk}^{(b)}$, with the verifier-facing pseudonym sampled
freshly as in the anonymity step, and the distinguisher $\mathcal A$ guesses $b$
with advantage $\bigl|\Pr[b'=b]-\tfrac12\bigr|$. The reduction mirrors the anonymity
proof, with the challenge bit selecting the pseudonym key rather than the
credential. Game $\mathsf{G}_0$ is the real experiment. In $\mathsf{G}_1$ the
challenge pseudonym signature under $\mathsf{psk}^{(b)}$ is produced by
$\mathcal{S}_\sigma$ in place of the secret key, so by Definition~\ref{def:szk}
$\bigl|\Pr[\mathsf{G}_0{=}1]-\Pr[\mathsf{G}_1{=}1]\bigr|\le\epsilon_{\mathrm{sZK}}$,
applied once to the single challenge transcript. In $\mathsf{G}_2$ the proof is
produced by $\mathcal{S}_\Pi$ in place of the witness, so by
Definition~\ref{def:zk}
$\bigl|\Pr[\mathsf{G}_1{=}1]-\Pr[\mathsf{G}_2{=}1]\bigr|\le\epsilon_{\mathrm{ZK}}$.
In $\mathsf{G}_2$ the only $b$-dependent component is the pseudonym signature, now
simulated without $\mathsf{psk}^{(b)}$, and the verifier-facing pseudonym is
independent of $b$, so $\Pr[b'=b]=\tfrac12$. Summing the two hops,
\[
  \mathsf{Adv}^{\mathsf{unlink}}_{\mathsf{zkVM\text{-}Cred}}(\mathcal A)
  \le \epsilon_{\mathrm{ZK}}+\epsilon_{\mathrm{sZK}},
\]
the same bound as~\eqref{eq:adv-anon}; the single-transcript game incurs each
simulation once, with no factor of two. One difference of presentation from BDEC's
A.3: there $c^{(b)}$ is a public-statement component, so its $\mathcal{S}_\sigma$
hop is forced, whereas in this thesis's relation $R^{\Sig}_{\mathsf{show}}$ the
credential $c_{U,V}$ is a witness component (Section~\ref{sec:prelim}), so the
$\mathcal{S}_\Pi$ hop already hides it and $\mathcal{S}_\sigma$ is needed only to
produce $c^{(b)}$ without $\sk_U$ inside the reduction. Either way
$\epsilon_{\mathrm{sZK}}$ enters once.
\end{proof}
...
PLUM is EU-CMA secure. Then the zkVM credential scheme is unforgeable, anonymous, and unlinkable.
