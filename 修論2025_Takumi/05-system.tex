...
that is \emph{irreducibly large-prime}, since its PRF security is $O(\sqrt p)$ key
recovery~\cite{khovratovich2019legendre,may2022legendre} and the prime cannot
shrink to a matched field the way the hash can (the SHA-3 control of
Section~\ref{sec:evaluation} shows the hash can), which makes it the natural target
