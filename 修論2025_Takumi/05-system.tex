...
Left: un-precompiled, each $\mathbb{F}_p^{199}$ operation expands into multi-limb
arithmetic and does not complete (OOM, $1$m$45$s). Right: a syscall routes it to a
native chip (Griffin AIR on SP1, \texttt{sys\_bigint} on RISC~Zero), bound by a
cross-table lookup and charged host-side---so a guest-cycle saving need not be a
wall-clock one.}
...
demonstrate that the interface \emph{generalises} across primitive
classes---a permutation, a field multiplication, and a composed modular
exponentiation---and, by Proposition~\ref{prop:field-match}, they also make the
criterion concrete in the negative direction: both re-implement arithmetic the
host \texttt{UINT256\_MUL} precompile \emph{already} serves (\texttt{FP192\_MUL} is
essentially that host chip retargeted to the $199$-bit modulus, and
\texttt{FP192\_POW\_RES} composes it), so the criterion predicts neither pays; for
\texttt{FP192\_POW\_RES} the trace-area accounting of
Section~\ref{sec:evaluation} confirms this directly. Griffin, by contrast, is
field-mismatched \emph{and} unserved by any existing precompile, which is why it
is the instance that pays. The interface is what makes three chips evidence for
one method rather than three unrelated artefacts.
