...
For PLUM ($\log_2 p\approx 199$) on BabyBear ($\lfloor\log_2 p_{\mathsf{vm}}\rfloor = 30$), $\ell = 7$, so Proposition~\ref{prop:fmt-lb} places the per-multiplication tax between $7$ and $49$. The measured precompile-free-to-precompiled cycle ratio of $56.4\times$ at $\lambda=80$ (Cell~1 / Cell~2, Section~\ref{sec:evaluation}) is consistent with the schoolbook $\ell^2\approx 49$ end realised across the hash-dominated workload, the modest residual reflecting the Griffin permutation's per-round structure beyond bare multiplication.
\end{remark}
